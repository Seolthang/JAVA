...
public class Exercise_RadioButtonItemEvent extends JFrame{
	...
	public Exercise_RadioButtonItemEvent() {
		...
		for(int i = 0; i < fruits.length; i++) {	
			...
			btnG.add(fruits[i]);					// 라디오 버튼 중 1개만 선택되도록 버튼 그룹 설정함.
			...
			fruits[i].addItemListener(new ItemListener(){
				...
				public void itemStateChanged(ItemEvent e) {
					...
					if(fruits[0].isSelected()){			// SetIcon() 메소드 사용
						imageLabel.setIcon(images[0]);  // Label 이미지를 Set해주는 메소드
		...
		c.add(radioPanel, BorderLayout.NORTH); // 라디오팬을 컨텐트팬에 추가함. 탑으로 붙여서 나옴
